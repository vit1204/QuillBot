...
import { ref } from 'vue'
import { replaceSelection } from '@/core'
...
const _popover = ref<HTMLElement>()

function handleReplaceText() {
  const text = props.replaceText[props.activeReplaceTextId]
  if (text && props.targetElement) {
    replaceSelection(props.targetElement, {
      start: 0,
      end: props.targetElement.textContent?.length || 0,
      text,
    })
  }

  emit('replaceSelectedText')
}
</script>
...
<template>
  ...
  >
    <div :class="$style.homePopoverHeader">
      ...
      <font-awesome-icon
        :class="$style.homeIcon" color="#A9A9A9" :icon="['fas', 'xmark']"
        @click="$emit('handleClosePopover')"
      />
      ...
      <div :class="$style.homePopoverContent">
        <div :class="$style.homePopoverContentHeader">
          ...
          <div :class="$style.homePopoverContentRefresh">
            <Icon
              :class="$style.homePopoverIcon" icon="material-symbols:chevron-left-rounded"
              color="#DADCE0" @click="$emit('handlePreReplacetext')"
            />
            ...
            <Icon
              ...
              color="#DADCE0" @click=" $emit('handleNextReplacetext') "
            />
          </div>
        </div>
      ...
      <div :class="$style.homePopoverBtnGroup">
        <button :class="$style.homePopoverCopyGroup" @click="$emit('handleCopyText')">
          Copy
        </button>
        <button @mousedown="handleReplaceText">
