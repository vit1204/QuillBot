...
const language = ref<string>('English (US)')
const languageList = [
  'English (US)',
  'Chinese',
  'Hindi',
...
const chilRects = ref<Array<DOMRect>>([])
...
const virtualElement = ref({
  ...
  getClientRect() {
    return chilRects.value[chilRects.value.length - 1]
  },
})
...
async function attachPopover() {
  if (popoverRef.value) {
    ...
    const max = window.innerWidth - 500
    const positionX = Math.min(x, max)

    popoverRef.value.style.position = strategy
    popoverRef.value.style.left = `${positionX + 200}px`
    ...
    <div :class="$style.Content">
      <div :class="$style.ContentParaPhrase">
        <div :class="$style.ContentLanguageList">
          <div
            v-for="(item, index) in languageList" v-show="index < 5" :key="index" :class="[
              (activeItem === index || (activeItem > 4 && index === 1))
                && $style.LanguageActiveItem,
              $style.ContentLanguageListItem,
            ]"
          >
            <span>{{
              index === 1 && activeItem > 4 ? languageList[activeItem] : item
            }}</span>
          </div>
        </div>
        <div :class="$style.TextAreaBox">
          ...
          <div :class="$style.TextArea">
            <div :class="$style.TextAreaLeftBox">
              <textarea
                id="" v-model="text" :class="$style.TextAreaTag" name="" cols="30" rows="25"
              ...
              <div :class="$style.TextAreaLeftFooter">
                ...
                <button v-if="!isFetching" :class="$style.TextAreaLeftButton" @click="parapharseText">
                  Paraphrase
                </button>

                <button v-else disabled :class="$style.TextAreaLeftButton">
                  Loading...
                </button>
              </div>
            </div>
            <div :class="$style.TextAreaRightBox">
              <div
                v-if="status === 'tooltip'" ref="tooltipRef" r :style="{
              ...
              >
                <img
                  width="20" height="20" src="https://sgroupvn.org/_nuxt/img/logo-sgroup-symbol.ae66e60.png"
                  alt=""
                >
              </div>

              <div
                v-else-if="status === 'popover'" ref="popoverRef" :style="{
                  backgroundColor: 'white',
                  width: '480px',
                  height: '278px',
                  borderRadius: '12px',
                ...
                <div :class="$style.PopoverContainer">
                  <div :class="$style.PopoverHeader">
                    <div :class="$style.PopoverHeaderTitle">
                      ...
                      <el-icon :class="$style.Icon" color="#A9A9A9" @click="handleClosePopover">
                        <Close />
                      </el-icon>
                    </div>
                    ...
                    <div :class="$style.PopoverContent">
                      <div :class="$style.PopoverContentHeader">
                        <div style="cursor: pointer;" :class="$style.PopoverContentRefresh" @click="getRefreshText">
                          ...
                          <Icon :class="$style.PopoverIcon" icon="basil:refresh-solid" color="#A9A9A9" />
                          <span>Refresh</span>
                        </div>
                        <div :class="$style.PopoverContentRefresh">
                          <el-icon :class="$style.PopoverIcon" color="#DADCE0" @click="handlePreReplacetext">
                            <ArrowLeft />
                          </el-icon>
                          ...
                          <el-icon :class="$style.PopoverIcon" color="#DADCE0" @click="handleNextReplacetext">
                            <ArrowRight />
                          </el-icon>
                        </div>
                      </div>
                    ...
                    <div :class="$style.PopoverBtnGroup">
                      <button :class="$style.PopoverCopyGroup" @click="handleCopyText">
                        Copy
                      </button>
              ...
              <div
                ref="inMouseArea" :class="$style.TextAreaTag" contenteditable
                placeholder="To rewrite text, enter or paste it here and press &quot;Paraphrase.&quot;" @blur="handleMouseBlur"
...
.ExpandLanguages {
  ...
  box-shadow:
    ...
    rgba(0, 0, 0, 0.04) 0px 16px 20px 0px,
